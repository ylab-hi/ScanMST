...
from scanmst import __PACKAGE_NAME__, cppext
from scanmst.base import Blat, Intervals, MappingMode, Strand
from scanmst.exception import ModesNotEqualError
...
def gene_annotation(
    ...
    try:
        gene1 = "&".join(sorted(gene_iv[HTSeq.GenomicPosition(chrm1, pos1)]))
    except IndexError:
        gene1 = ""
    except TypeError:
        pass
    ...
    try:
        gene2 = "&".join(sorted(gene_iv[HTSeq.GenomicPosition(chrm2, pos2)]))
    except IndexError:
        gene2 = ""
    except TypeError:
        pass
...
def splicing_confirmation_and_correction(
    ...
    *,
    ...
    """Judge whether the breakpoints are MST events or not.

    if motif_required is ON: it will only report MST events with 'canonical
    splice sites'; otherwise: it will report MST events whatever the splice
    sites they used
...
def _extract_read_sequence_from_matched_segment(
    soft_seq_original: str,
    seq_original: str,
    read_strand: str,
    read_mode: MappingMode,
    potential_microhomology_length: int,
) -> str:
    """
    Extract the sofclipping boundary read sequence in the matched segment.

    :param soft_seq_original: softclipped segment of the aligned read (original sequence)
    ...
    :param potential_microhomology_length: potential microinsertion length
    """
...
def blat2chimeric_alignment(
    ...
    potential_microhomology_length = rt_switching_filter_len + 5
    ...
    head_match_pattern = re.compile(r"^\d+M")
    tail_match_pattern = re.compile(r"\d+M$")
    read_hom_seq = _extract_read_sequence_from_matched_segment(
        soft_seq_original,
        seq_original,
        read_strand,
        read_mode,
        potential_microhomology_length,
    )
    ...
    if isinstance(aligner, Blat):
        ...
        if flag:
            ...
            pos_end_sa = insertion_info.ref_end
            strand_sa = str(insertion_info.strand)
            ...
            if mapq >= mapq_cutoff and int(nm_sa) < max_allowed_nm:
                return f"{chrom_sa},{pos_start_sa + 1},{strand_sa},{valid_cigar_sa},{mapq},{nm_sa};"
    ...
    if flag:
        ...
        if (
            ...
            if nm_sa < max_allowed_nm:
                chimeric_aln_str = f"{chrom_aligner},{original_ref_start + 1},{read_strand},{valid_cigar_sa},{mapq_aligner},{nm_sa};"
...
def same_chrom_diff_strand_handler(
    ...
    if ra_bp == sa_bp:  # inverted duplication (IDUP)
        ...
        if not read_lt.splice_site_checker(
            ...
            logger.debug(
                f"Splice site checking[IDUP]: {read_lt.query_name=}, {read_lt.cigarstring=}, {read_rt.cigarstring=}",
            )
            return noreturn
        chrm_start = lt_chrm
        junc_start = ra_bp
