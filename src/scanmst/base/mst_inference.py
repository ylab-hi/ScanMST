...
def infer_mst_from_connected_reads(
    read_lt,
    read_rt,
    lt_mode: int,
    rt_mode: int,
    ...
    *,
    ...
    """Infer MST event from connected reads.
    ...
    :param microinsertion_cutoff: threshold of dicarding MST event with long microinsertion (>60bp)
    ...
    if lt_chrm == rt_chrm:
        if lt_strand == rt_strand:  # deletion, insertion, duplication
            ...
            if not read_lt.splice_site_checker(
                ...
                logger.debug(
                    f"Splice site checking[Same chroms, same strands]: {read_lt.query_name=}, {read_lt.cigarstring=}, {read_rt.cigarstring=}",
                )
                return noreturn
    ...
    if not read_lt.splice_site_checker(
        ...
        logger.debug(
            f"Splice site checking[different chroms]: {read_lt.query_name=}, {read_lt.cigarstring=}, {read_rt.cigarstring=}",
        )
        return noreturn
    ...
    # lt_strand not same as rt_strand
