...
from scanmst import cppext
from scanmst.exception import ReadNotFoundError
...
class Event:
    """Event class is used to parse the return value from the function mst_inference.

    :Example:

    >>> args, kwargs = [], {}
    >>> event = Event(infer_mst_from_connected_reads(*args, **kwargs))
    >>> event.sv_type
    TRA
    >>> event
    Event(TRA, )
