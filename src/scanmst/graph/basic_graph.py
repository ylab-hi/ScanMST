...
if TYPE_CHECKING:
    ...
    from scanmst.base.basic_read import Read
...
class Node(BasicNode):
    ...
    def merge(
        ...
        if isinstance(other, Node):
            ...
            # WARN: ref_end may be not consistent with prev_breakpoint of next edge
            ...
            # WARN: do not check if they have same key
...
class Edge:
    ...
    def merge(self, other: Edge, pnode_strand: Strand, nnode_strand: Strand, *, merge_insertion_info=True):
        # WARN: update breakpoint in cmparing way
        ...
        for index, event in enumerate(events):
            ...
            if event.has_insertion():
                ...
                if flag:  # keep the top one hit, mapq does not work here
                    ...
                    logger.trace("mst reference for read1 and insertion")

                    read1_insertion_event = infer_mst_from_connected_reads(
                        read_lt=read1,
                        read_rt=insertion,
                        lt_mode=event.mode1,
                    ...
                    logger.trace("mst reference for read2 and insertion")
                    insertion_read2_event = infer_mst_from_connected_reads(
                        read_lt=insertion,
                        read_rt=read2,
