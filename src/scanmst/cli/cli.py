...
def cli(options: argparse.Namespace | DefaultOptions):
    ...
    logger.info(f"scanmst starts running in {running_mode} mode PID-{os.getpid()}")
    ...
    try:
        ...
        logger.info(f"ScanMST takes {time.perf_counter() - start:.2f} seconds.")
