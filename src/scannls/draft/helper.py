...
def similarity_distance(series_a: Series, series_b: Series) -> float:
    """calculate the similarity distance between two series, using longer one as the reference
    :param series_a: Series A, reference
    :param series_b: Series B, query
    :type series_a: Series object
    :type series_b: Series object
    :return: calculated similarity distance
    :rtype: float [0, 1]
    """

    def obtain_breakpoint_pairs(series):
    ...
    def breakpoints_distance(sv_type1: str, sv_type2: str, bp1: str, bp2: str) -> int:
        """calculate breakpoint distance
            sv_type1,chrA:pos1 VS sv_type2,chrB:pos2
    ...
    def first_node_last_node_distance(first_node: Node, last_node: Node) -> float:
        ...
        :type first_node: Node object
        :type last_node: Node object
        :return: calculated breakpoint distance
        :rtype: int
        ...
        if _ft_strand != _lt_strand:
            ...
            if _ft_strand == _lt_strand == "+":
                if (
                    first_node.exons[0][0] >= last_node.exons[0][0]
                    and first_node.exons[-1][1] >= last_node.exons[-1][1]
                    and last_node.exons[-1][1] > first_node.exons[0][0]
                ):
                    overlapped_len = last_node.exons[-1][1] - first_node.exons[0][0]

                    _ft_cov = overlapped_len / (
                        first_node.exons[-1][1] - first_node.exons[0][0]
                    )
                    _lt_cov = overlapped_len / (
                        last_node.exons[-1][1] - last_node.exons[0][0]
                    )
            ...
            #         [xxxx]--<--
                ...
                if (
                    first_node.exons[0][0] <= last_node.exons[0][0]
                    and first_node.exons[-1][1] <= last_node.exons[-1][1]
                    and first_node.exons[-1][1] > last_node.exons[0][0]
                ):
                    overlapped_len = first_node.exons[-1][1] - last_node.exons[0][0]
        ...
        for i, j in zip(bp_pair1, bp_pair2):
            ...
            if a_sv_type != "NA" and b_sv_type != "NA":
                distance_list.append(
                    breakpoints_distance(a_sv_type, b_sv_type, a_bp1, b_bp1)
                )
                distance_list.append(
                    breakpoints_distance(a_sv_type, b_sv_type, a_bp2, b_bp2)
                )
                effect_num_pair += 1
    ...
    # always use the longer series as the reference
    if len(series_a) < len(series_b):
        series_a, series_b = series_b, series_a
    series_a_bp_pair = obtain_breakpoint_pairs(series_a)
    series_b_bp_pair = obtain_breakpoint_pairs(series_b)

    calculated_distance_list = []
    # ref:           [x]-[x]-[x]-[x]
    # query: [x]-[x]-[x]
    distance = first_node_last_node_distance(series_a[0], series_b[-1])
    ...
    for i in range(sliding_window_size - 1):
        series_a_bp_pair.insert(0, ("NA", "chrN:0", "chrN:0"))
        series_a_bp_pair.append(("NA", "chrN:0", "chrN:0"))
    ...
    for i in range(len(series_a_bp_pair) - sliding_window_size + 1):
        ...
        if i >= sliding_window_size:
            left_subject_node = series_a[i + 1 - sliding_window_size]
        else:
            left_subject_node = None
        if (
            i >= 0
            and i
        ...
        if right_subject_node:
            if (
                ...
                flag = False
        if flag:
            distance = breakpoint_pairs_distance(series_b_bp_pair, _subject_bp_pair)
            calculated_distance_list.append(distance)

    # ref:    [x]-[x]-[x]-[x]
    # query:              [x]-[x]-[x]
    distance = first_node_last_node_distance(series_b[0], series_a[-1])
    calculated_distance_list.append(distance)
    if calculated_distance_list:
        return min(calculated_distance_list)
    else:
        return 1.0
