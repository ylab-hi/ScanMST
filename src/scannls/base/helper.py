    ...
    if isinstance(aligner, Blat):
        ...
        if flag:
            ...
            if read_strand == strand_sa:
                ...
                # SM
                cigar_sa = (
                    f"{read_length - in_seq_len}S{cigar_sa_partial}" if read_mode == 1 else f"{cigar_sa_partial}{read_length - in_seq_len}S"
                )  # MS
            else:
                ...
                # MS
                cigar_sa = (
                    f"{cigar_sa_partial}{read_length - in_seq_len}S" if read_mode == 1 else f"{read_length - in_seq_len}S{cigar_sa_partial}"
                )  # SM
    ...
    if flag:
        ...
        if (
            ...
            if nm_sa < max_allowed_nm:
                chimeric_aln_str = f"{chrom_aligner},{original_ref_start + 1}," f"{read_strand},{valid_cigar_sa},{mapq_aligner},{nm_sa};"
