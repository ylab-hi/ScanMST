...
def start_gfServer(ref_2bit, timeout=300, port=88888, output_dir='/tmp'):
    ...
    sys.stderr.write('Current directory: {}\n'.format(os.getcwd()))
    try:
        cmd = 'gfServer -canStop -log={0}/gfserver.temp.log -stepSize=5 start localhost {1} {2} &'.format(log_dir, port, base_2bit)
        print(cmd)
        start = time.time()
        ret = subprocess.check_call(cmd,  stderr=subprocess.STDOUT, shell=True)
    except subprocess.CalledProcessError as err:
        print('Execution failed for starting BLAT gfServer:', err, err.output, file=sys.stderr)
        sys.exit(1)
    else:
        if not ret:
            interval = 10
            ...
            sys.stderr.write('Something wrong in {0}/gfserver.temp.log\n'.format(output_dir))
    os.chdir(cwd)
    sys.stderr.write('Current directory: {}\n'.format(os.getcwd()))

def stop_gfServer(port=88888, output_dir='/tmp'):
    try:
        subprocess.check_call('gfServer stop localhost {0}'.format(port), stderr=subprocess.STDOUT, shell=True)
...
def gfClient_query(in_seq, ref_2bit, port=88888, output_dir='/tmp'):
    ...
    #print(os.getcwd())
    try:
        cmd = 'gfClient -minScore=20 -minIdentity=0 localhost {} {} {} {} > /dev/null'.format(port, ref_dir, in_fasta, out_psl)
        #print(cmd)
...
def blat_mapq_calculator(hsps, query_len, blat_ident_pct_cutoff = 0.95):
    ...
    num_of_locations = 0
    for hsp in hsps:
        if hsp.ident_pct / 100 >= blat_ident_pct_cutoff and hsp.query_span / query_len >= blat_ident_pct_cutoff:
            num_of_locations += 1
...
def softclipped_seq2SA_tag(in_seq, read_length, read_strand, read_mode, ref_2bit, port, mapq_cutoff, max_allowed_nm, output_dir='/tmp', blat_ident_pct_cutoff = 0.95):
    '''
    ...
    try:
        ...
        if top_hsp.ident_pct / 100 >= blat_ident_pct_cutoff and top_hsp.query_span / in_seq_len >= blat_ident_pct_cutoff:
            __chrm_sa, __pos_sa, __strand_sa, __cigar_sa_partial, __nm_sa =  psl2sam(top_hsp, in_seq_len)
            if read_strand == __strand_sa:
                # same strand: different reads mode
                # MS(1) ~ SM(2) or SM(2) ~ MS(1)
                if read_mode == 1:
                    __cigar_sa = '{0}S{1}'.format(read_length - in_seq_len, __cigar_sa_partial) #SM
                else:
                    __cigar_sa = '{1}{0}S'.format(read_length - in_seq_len, __cigar_sa_partial) #MS
            else:
            ...
            if __mapq < mapq_cutoff and int(__nm_sa) < max_allowed_nm:
                chimeric_aln_str = '{},{},{},{},{},{};'.format(__chrm_sa, __pos_sa, __strand_sa, valid_cigar_sa, __mapq, __nm_sa)
            else:
                chimeric_aln_str = ''
