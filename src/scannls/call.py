...
def sv_scan(input_bam, target, output_prefix, sr_cutoff, dp_cutoff, pso_cutoff, sv_len_cutoff, soft_len_cutoff, mapq_cutoff, mismatch_cutoff, alignment_frac, seed_cutoff):
    ...
    try:
        with open(target) as ifp:
            for line in ifp:
                chrm1, s1, e1, chrm2, s2, e2, group_id, sv_ao, strand1, strand2 = line.rstrip('\n').split('\t')
                if not '_' in chrm1 and not '_' in chrm2:
                    regions[(f'{chrm1}:{s1}-{e1}', f'{chrm2}:{s2}-{e2}')] = {'ao': int(sv_ao), 'group': group_id}
    ...
    for region_l, region_r in regions:
        ...
        if region_l == region_r:
            ...
            ins_gene_dict = {}
            try:
                for col in bam_file.pileup(region=region_l, truncate=True, stepper='nofilter', min_base_quality=0):
                    #print(col.reference_name, col.reference_pos)
                    chrm = col.reference_name
                    dp = col.nsegments
                    for read in col.pileups:
                        # read is an instance of pysam.PileupRead
                        if read.alignment.mapq >= mapq_cutoff and read.query_position != None:
                            if read.alignment.has_tag('OT'):
                                sv_type, _anno_can, _position, size_or_sup_position, rep_aln_mode, sup_aln_mode, strands, genes = read.alignment.get_tag('OT')[:-1].split(',')
                                if sv_type == 'INS':
                                    ref_allele, alt_allele = _anno_can.split('|')
                                    sv_id = '{},{},{},{}'.format(sv_type, _position, ref_allele, alt_allele)
                    ...
                    for sv_id in ins_ao_dict:  # only consider one event per genomic position
                        if sv_id in sv_set:
                            continue
                        ...
                        if dp >= dp_cutoff and ao >= sr_cutoff and vaf >= pso_cutoff:
                            ...
                            vcf_field_gt = 'GT\t0/1'
                            vcf_field_info = '{};AO={};DP={};VAF={:.2g};SVLEN={};SVTYPE={};SVMETHOD=ScanNLS;GENE1={};GENE2={};STRAND1={};STRAND2={};END={}'.format(can_field,ao,dp,vaf,bp_distance,sv_type,bp1_gene,bp2_gene,bp1_strand,bp2_strand,_position)
                            vcf_file.write('{}\t{}\tINS_{}\t{}\t{}\t.\t.\t{}\t{}\n'.format(chrm, _position, INS_count, ref_allele, alt_allele, vcf_field_info, vcf_field_gt))
                            INS_count += 1
                        ...
                        if read.alignment.mapq >= mapq_cutoff and read.query_position != None:
                            ...
                            if not read.alignment.has_tag('SV'):
                                if 'S' in read.alignment.cigarstring:
                                    #print(read.alignment.query_name)
                                    soft_len, soft_seq, soft_pos, soft_mode = get_softclip_length(read.alignment)
                                    #print(soft_mode, read.alignment.query_name)
                                    if 'N' not in soft_seq:
                                        if soft_len >= soft_len_cutoff:
                                            if soft_mode == 1:
                                                if abs(len(read.alignment.query_sequence[read.query_position + 1:]) - soft_len) < 20:
                                                    sr_list[1].append(read.alignment.query_sequence[read.query_position + 1:])
                                            elif soft_mode == 2:
                                                if abs(len(read.alignment.query_sequence[:read.query_position]) - soft_len) < 20:
                                                    sr_list[2].append(read.alignment.query_sequence[:read.query_position])
                            else:
                                if sv_checker(read, col.reference_pos, mapq_cutoff, sv_len_cutoff):
                                    _, _, _, sv_soft_mode = get_softclip_length(read.alignment)
                                    sv_type, _anno_can, _position, size_or_sup_position, rep_aln_mode, sup_aln_mode, strands, genes = read.alignment.get_tag('SV')[:-1].split(',')
                                    # store gene information
                                    ...
                                    if is_bps_order_changed(f'{chrm}:{_position}', f'{_chrm2}:{_bp2_pos}'):
                                        if _anno_can[0] == '1':
                                            _anno_can = f'2{_anno_can[1]}'
                                        elif _anno_can[0] == '2':
                                            _anno_can = f'1{_anno_can[1]}'
                                        sv_id = '{},{},{},{},{}:{}'.format(sv_type, _anno_can, _chrm2, _bp2_pos, chrm, _position)
                                        strand_dict[sv_id] = (strands[1], strands[0])
                                        gene_dict[sv_id] = (_gene2, _gene1)
                                        if abs(int(_position) - col.reference_pos) < abs(int(_bp2_pos)-col.reference_pos):
                                            mode_dict[sv_id] = (int(sup_aln_mode), int(rep_aln_mode))
                                        else:
                                            mode_dict[sv_id] = (int(rep_aln_mode), int(sup_aln_mode))
                                    else:
                                        #sv_id = '{},{},{},{},{}'.format(sv_type, _anno_can, chrm, _position, size_or_sup_position)
                                        sv_id = '{},{},{},{},{}:{}'.format(sv_type, _anno_can, chrm, _position, _chrm2, _bp2_pos)
                                    ...
                                    if int(rep_aln_mode) == 1:
                                        sv_seq_dict[sv_id].add(read.alignment.query_sequence[read.query_position + 1:])
                                    # SM mode (representive alignment)
                                    else:
                                        sv_seq_dict[sv_id].add( read.alignment.query_sequence[:read.query_position] )

                                    if not sv_id in ao_dict:
                                        ao_dict[sv_id] = 1
                                    else:
                                        ao_dict[sv_id] += 1
                    for sv_id in ao_dict:  # only consider one event per genomic position
                        if sv_id in sv_set:
                            continue
                        ao = ao_dict[sv_id]
                        if ao < seed_ao_cutoff:
                        ...
                        if sv_soft_mode in sr_list:
                            for sr_read in sr_list[sv_soft_mode]:
                                ...
                                if mismatch_count(sr_read, sv_seq_dict[sv_id], alignment_frac, sv_soft_mode) <= mismatch_cutoff:
                                    ao += 1
                        #print('Left AO:', ao)
            ...
            ##########right breakpoint######################################
            try:
                for col in bam_file.pileup(region=region_r, truncate=True, stepper='nofilter', min_base_quality=0):
                    dp_r = col.nsegments
                    chrm = col.reference_name
                    ao_dict = {}
                    sv_seq_dict = defaultdict(set)
            ...
            for sv_id in ao_total_dict:
                ...
                try:
                    dp1 = bam_file.count(region='{}:{}-{}'.format(bp1_chrm, bp1_pos-1, bp1_pos))
                except ValueError as e:
                    print(f'{e} at {bp1_chrm}:{bp1_pos}')
                    dp1 = 0
                try:
                    dp2 = bam_file.count(region='{}:{}-{}'.format(bp2_chrm, bp2_pos-1, bp2_pos))
                except ValueError as e:
                    print(f'{e} at {bp2_chrm}:{bp2_pos}')
                    dp2 = 0
                pso = sr/(sr + (dp1+dp2)/2)
                if (dp1+dp2)/2 >= dp_cutoff and sr >= sr_cutoff and pso >= pso_cutoff:
                    sv_set.add(sv_id)
                    ...
                    vcf_field_gt = 'GT\t0/1'
                    vcf_field_info = '{};SR={};DP1={};DP2={};PSO={:.2g};SVLEN={};SVTYPE={};BOUNDARY={};SVMETHOD=ScanNLS;GENE1={};GENE2={};STRAND1={};STRAND2={};MODE1={};MODE2={};CHR2={};END={}'.format(can_field,sr,dp1,dp2,pso,bp_distance,sv_type,anno_field,bp1_gene,bp2_gene,bp1_strand,bp2_strand,mode1,mode2,bp2_chrm,bp2_pos)
                    vcf_file.write('{}\t{}\tNLS_{}\t.\t<{}>\t.\t.\t{}\t{}\n'.format(bp1_chrm, bp1_pos, ID_count, sv_type, vcf_field_info, vcf_field_gt))
                    ID_count += 1
    bam_file.close()
    return True
