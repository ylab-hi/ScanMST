...
def mismatch_filter(anchor_seq, anchor_soft_pos, in_list, alignment_frac, mismatch_cutoff):
    out_list = []
    ...
    for _mode, _seq, _pos, _read in in_list:
        ...
        # raise IndexError if SSW cannot work
            ...
            continue
        if len(alignment[0])/float(len(anchor_seq)) < alignment_frac and len(alignment[1])/float(len(each_seq)) < alignment_frac:
            continue
        # Align left
        ...
        # Align right
        elif start_end_pos[0][1] == len(anchor_seq) - 1 and start_end_pos[1][1] == len(each_seq) - 1:
            if sum(alignment[0].mismatches(alignment[1])) < mismatch:
                mismatch = sum(alignment[0].mismatches(alignment[1]))
        else:
            continue
    ...
    if read.cigar[0][0] == 4:
        ...
        if read.cigar[-1][0] == 4:
            ...
            if read.cigar[0][1] > read.cigar[-1][1]:
                return read.cigar[0][1], read.query_sequence[:read.cigar[0][1]], read.reference_start, 2
            # length of right soft-clipped segment is bigger
            else:
                return read.cigar[-1][1], read.query_sequence[read.query_length - read.cigar[-1][1]:], read.reference_end - 1, 1
...
def get_softclip_length_from_cigar(cigar_str_sa, ref_start_sa, strand_sa, strand_ra, query_sequence_ra):
    '''
       return: length of soft-clipped part, sequence of soft-clipped part, the connection point of soft-clipped part (left/right), left/right soft-clipped part: 0:other; 2:left[SM]; 1:right[MS]
    '''
...
def measure_size(cigartuple):
    ...
    for each_type in cigartuple:
        if each_type[1] == 'I':
            ...
            cigar.append([2, int(each_type[0])]) if each_type[1] == 'D' else cigar.append(
                [3, int(each_type[0])])
        elif each_type[1] == 'M':
            match_size += int(each_type[0])
...
def multiple_sa_tag_selector(sa_tag_list):
    ...
    span_nm_max_mapq_dict = dict((j, span_nm_dict[j]) for j in mapq_dict if mapq_dict[j] == max_mapq)
...
def sequence_extracter(cigar, chrm, strand, pos, genome_fasta):
    ...
    if strand == '+':
        ...
        for __item in exon_positions[::-1]:
            seq = genome_fasta[__item.chrom][__item.start:__item.end].reverse.complement.seq
            out_seq += seq
...
def obtain_seq_from_interval_list(in_list, genome_fasta):
    ...
    if strand == '+':
        ...
        for _item in in_list[::-1]:
            seq = genome_fasta[_item.chrom][_item.start:_item.end].reverse.complement.seq
            out_seq += seq
    return out_seq
...
def candidates_dict_to_GTF_contents(candidates_gtf_count_dict, gene_id, event_type, bp_pos1, bp_pos2, genome_fasta):
    '''
    '''
    gtf_list = []
    count = 1
    ...
    for candidate in candidates_gtf_count_dict:
        ...
        if up_stream and down_stream:
            gtf_list.append(f'{up_stream[0].chrom}\tScanNLS\ttranscript\t{up_stream[0].start+1}\t{up_stream[-1].end}\t.\t{up_stream[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.{count}"; event_type "{event_type}"; segment "upstream"; support_reads "{supporting_reads}";')
            for exon in up_stream:
                gtf_list.append(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.{count}"; event_type "{event_type}"; segment "upstream";')
                up_total_len += exon.end - exon.start
            gtf_list.append(f'{down_stream[0].chrom}\tScanNLS\ttranscript\t{down_stream[0].start+1}\t{down_stream[-1].end}\t.\t{down_stream[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.{count}"; event_type "{event_type}"; segment "downstream"; support_reads "{supporting_reads}";')
            for exon in down_stream:
                gtf_list.append(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.{count}"; event_type "{event_type}"; segment "downstream";')
            ...
            fasta_seq = obtain_transcript_seq_from_up_down_tuple(candidate, genome_fasta)
...
def exon_positions_to_GTF_content(event_exons, gene_id, event_type):
    ...
    out_list = []
    out_list.append(f'{up_stream[0].chrom}\tScanNLS\ttranscript\t{up_stream[0].start+1}\t{up_stream[-1].end}\t.\t{up_stream[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.1"; event_type "{event_type}";')
    for exon in up_stream:
        out_list.append(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.1"; event_type "{event_type}";')
        up_total_len += exon.end - exon.start
    out_list.append(f'{down_stream[0].chrom}\tScanNLS\ttranscript\t{down_stream[0].start+1}\t{down_stream[-1].end}\t.\t{down_stream[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.2"; event_type "{event_type}";')
    for exon in down_stream:
        out_list.append(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{gene_id}.2"; event_type "{event_type}";')
        down_total_len += exon.end - exon.start
    total_len = up_total_len + down_total_len
...
def update_cigar(chrm, ref_start, strand, cigar_str, tgt_breakpoint, genome_fasta, is_can=True):
    ...
    return cigartuple2cigarstring(cigartuple_list), cigartuple2cigartuple_in_reads(cigartuple_list), updated_ref_start
...
def correct_breakpoint(chrm, ref_start, cigar_str, strand_sa, strand_ra, soft_seq, genome_fasta):
    ...
    if soft_mode == 2:
        ...
        if shorter_flag:
            ...
            shift_len = -(updated_ref_end - _matched_size + (_total_len - SOFT_LEN) - updated_ref_start)
        ...
        # SOFT_LEN > _total_len
            ...
            _ref_seq = genome_fasta[chrm][soft_bp_pos - (diff_len+10):soft_bp_pos+10].seq
            ## print('_soft_seq:', _soft_seq)
            ## print('_ref_seq:', _ref_seq)
            __shift_len_L, __shift_len_R, __aln_len = aln_to_position(_soft_seq, _ref_seq)
    ...
    # MS
        ...
        if shorter_flag:
            ...
            print(soft_bp_pos)
            _ref_seq = genome_fasta[chrm][soft_bp_pos-10:soft_bp_pos+diff_len+10].seq
            __shift_len_L, __shift_len_R, __aln_len = aln_to_position(_soft_seq, _ref_seq)
            print('_soft_seq:', _soft_seq)
            print('_ref_seq:', _ref_seq)
...
def obtain_upstream_breakpoint_seq(chrm1, chrm2, pos1, pos2, strand1, strand2, mode1, mode2, genome_fasta):
    '''
...
def refine_upstream_downstream_breakpoint(in_up_dict, in_down_dict, chr1, chr2, pos1, pos2, strand1, strand2, mode1, mode2):
    '''
    '''
    up_chrm = ''
    up_pos = 0
...
def vcf_construction(in_vcf, in_bam, out_gtf, ref_genome, output_prefix):
    ...
    for record in vcf_reader:
        ...
        transcript_seq_l = ''
        transcript_exons_l = ()
        for pileupcolumn in bam_file.pileup(region=region_l, truncate=True, stepper='all', min_base_quality=0):
            ...
            for read in pileupcolumn.pileups:
                ...
                if read_ra.has_tag('SV'):
                    sv_type, _can, position, size_or_sup_position, rep_aln_mode, sup_aln_mode, strands, genes = read_ra.get_tag('SV').split(',')
                    bp1 = '{}:{}'.format(chrm, position)
                    if ':' in size_or_sup_position:
                        bp2 = size_or_sup_position
                    else:
                        bp2 = '{}:{}'.format(chrm, int(position)+int(size_or_sup_position))
                    if sv_type == _sv_type and (bp1 == _bp1 and bp2 == _bp2) or (bp1 == _bp2 and bp2 == _bp1):
                        soft_len, soft_seq, soft_pos, soft_mode = get_softclip_length(read_ra)
                        new_pos1 = soft_pos
                        if soft_mode == 1:
                            new_pos1 = new_pos1 + 1
                        search_seq = soft_seq
                        ...
                        if len(chimeric_aln) == 1:
                            chrm_sa, pos_sa, strand_sa, cigar_sa, mapq_sa, nm_sa = chimeric_aln[0].split(',')
                        else:
                            chrm_sa, pos_sa, strand_sa, cigar_sa, mapq_sa, nm_sa = multiple_sa_tag_selector(chimeric_aln).split(',')

                        soft_mode = cigarstring2mode(cigar_sa)
                        pos_sa = int(pos_sa) - 1

                        new_pos2 = correct_breakpoint(chrm_sa, pos_sa, cigar_sa, strand_sa, strand_ra, search_seq, genome_fasta)

                        if not new_pos2:
                            soft_len_sa, soft_seq_sa, soft_pos_sa, soft_mode_sa = get_softclip_length_from_cigar(cigar_sa, pos_sa, strand_sa, strand_ra, read_ra.query_sequence)
                            new_pos2 = soft_pos_sa
                            new_pos2 = new_pos2 - 1
                            search_seq = soft_seq_sa
                            if soft_mode_sa == 1:
                                new_pos2 = new_pos2 + 1
                            new_pos1 = correct_breakpoint(read_ra.reference_name, read_ra.reference_start, read_ra.cigarstring, strand_sa, strand_ra, search_seq, genome_fasta)
                        print('new left positions: ', new_pos1, new_pos2)
                        #############################################################################################
                        updated_cigarstring, updated_cigar, updated_reference_start \
                             = update_cigar(chrm, \
                             read.alignment.reference_start, strand1, read.alignment.cigarstring, new_pos1, genome_fasta, is_can)
                        if updated_reference_start:
                            read.alignment.cigarstring, read.alignment.cigar, read.alignment.reference_start \
                                = updated_cigarstring, updated_cigar, updated_reference_start
                            left_seq, left_exons = sequence_extracter(read_ra.cigarstring, read_ra.reference_name, strand_ra, read_ra.reference_start, genome_fasta)
                        updated_cigarstring_sa, _, updated_reference_start_sa \
                            = update_cigar(chrm_sa, pos_sa, strand_sa, cigar_sa, new_pos2, genome_fasta, is_can)

                        if updated_reference_start_sa:
                            new_cigar_sa, new_pos_sa = updated_cigarstring_sa, updated_reference_start_sa
                            new_pos_sa = int(new_pos_sa)
                            right_seq, right_exons = sequence_extracter(new_cigar_sa, chrm_sa, strand_sa, new_pos_sa, genome_fasta)
        ...
        transcript_exons_r = ()
        # right_breakpoint
        for pileupcolumn in bam_file.pileup(region=region_r, truncate=True, stepper='all', min_base_quality=0):
            dp = pileupcolumn.nsegments
            chrm = pileupcolumn.reference_name
            sv_soft_segments = []
            soft_segments = []
            for read in pileupcolumn.pileups:
                ...
                if read_ra.has_tag('SV'):
                    ...
                    if sv_type == _sv_type and (bp1 == _bp1 and bp2 == _bp2) or (bp1 == _bp2 and bp2 == _bp1):
                        ...
                        new_pos1 = correct_breakpoint(chrm_sa, pos_sa, cigar_sa, strand_sa, strand_ra, search_seq, genome_fasta)
                        ...
                        if not new_pos1:
                            ...
                            if soft_mode_sa == 1:
                                new_pos1 = new_pos1 + 1
                            new_pos2 = correct_breakpoint(read_ra.reference_name, read_ra.reference_start, read_ra.cigarstring, strand_sa, strand_ra, search_seq, genome_fasta)
                        print('new right positions: ', new_pos1, new_pos2)
                        ########################################################################
                        updated_cigarstring, updated_cigar, updated_reference_start \
                             = update_cigar(chrm, \
                             read.alignment.reference_start, strand2, read.alignment.cigarstring, new_pos2, genome_fasta, is_can)
                        if updated_reference_start:
                            read.alignment.cigarstring, read.alignment.cigar, read.alignment.reference_start \
                                = updated_cigarstring, updated_cigar, updated_reference_start
                            right_seq, right_exons = sequence_extracter(read_ra.cigarstring, read_ra.reference_name, strand_ra, read_ra.reference_start, genome_fasta)

                        updated_cigarstring_sa, _, updated_reference_start_sa \
                            = update_cigar(chrm_sa, pos_sa, strand_sa, cigar_sa, new_pos1, genome_fasta, is_can)
                        if updated_reference_start_sa:
                            new_cigar_sa, new_pos_sa = updated_cigarstring_sa, updated_reference_start_sa
                            new_pos_sa = int(new_pos_sa)
                            left_seq, left_exons = sequence_extracter(new_cigar_sa, chrm_sa, strand_sa, new_pos_sa, genome_fasta)
        ...
        if candidates_gtf_count_dict:
            gtf_record_list, fasta_dict = candidates_dict_to_GTF_contents(candidates_gtf_count_dict, gene_id, _sv_type, _bp1, _bp2, genome_fasta)
            # Output GTF
...
def interval_list_merger(interval_list, shrink=False):
    ...
    if tot_len == 1:
        ...
        if tot_len > 30 and shrink:
            mid = int(tot_len/2)
            indices = [0,1,2,mid,mid+1,mid+2,tot_len-3,tot_len-2,tot_len-1]
...
def MSA_to_consensus(in_fasta, program='Mafft'):
    ...
    try:
        #print(cline)
        return_code = subprocess.check_call(cline, shell=True, stderr=subprocess.STDOUT, stdin=subprocess.PIPE)
        #print(stdout)
...
def vcf_parser(in_vcf, pos_dict, in_bam, out_gtf, ref_genome, alignment_frac, mismatch_cutoff):
    try:
        genome_fasta = Fasta(ref_genome, sequence_always_upper=True)
    except FastaNotFoundError as e:
        print('read reference genome ' + ref_genome + ' error!', e)
    ...
    for record in vcf_reader:
        ...
        if new_pos1 > 0 and new_pos2 > 0:
            ...
            #Left breakpoint
            transcript_seq_l, transcript_exons_l = transcript_contruction(bam_file, chrm1, chrm2, pos1, pos2, \
                    new_pos1, new_pos2, strand1, strand2, mode1, mode2, _sv_type, genome_fasta, alignment_frac, mismatch_cutoff, left=True)
            #Right breakpoint
            transcript_seq_r, transcript_exons_r = transcript_contruction(bam_file, chrm1, chrm2, pos1, pos2, \
                    new_pos1, new_pos2, strand1, strand2, mode1, mode2, _sv_type, genome_fasta,alignment_frac, mismatch_cutoff,left=False)
            ...
            gtf_record_list = exon_positions_to_GTF_content(transcript_exons, gene_id, _sv_type)
...
def update_gtf(in_gtf, out_gtf, out_fasta, ref_genome):
    ...
    for gene_id in gene_id_list:
        ...
        # output to GTF file
        out_gtf_file.write(f'{_up[0].chrom}\tScanNLS\ttranscript\t{_up[0].start+1}\t{_up[-1].end}\t.\t{_up[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{trx_upstream}"; event_type "{event_type}";\n')
        for exon in _up:
            out_gtf_file.write(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{trx_upstream}"; event_type "{event_type}";\n')
            upstream_titles.append(f'{exon.chrom}:{exon.start+1}-{exon.end}({exon.strand})')
        out_gtf_file.write(f'{_down[0].chrom}\tScanNLS\ttranscript\t{_down[0].start+1}\t{_down[-1].end}\t.\t{_down[0].strand}\t.\tgene_id "{gene_id}"; transcript_id "{trx_downstream}"; event_type "{event_type}";\n')
        for exon in _down:
            out_gtf_file.write(f'{exon.chrom}\tScanNLS\texon\t{exon.start+1}\t{exon.end}\t.\t{exon.strand}\t.\tgene_id "{gene_id}"; transcript_id "{trx_downstream}"; event_type "{event_type}";\n')
            downstream_titles.append(f'{exon.chrom}:{exon.start+1}-{exon.end}({exon.strand})')
        bp_up = ''
        bp_down = ''
        ...
        if _up[0].strand == '+':
            ...
            for exon in _up[::-1]:
                upstream_seq += genome_fasta[exon.chrom][exon.start:exon.end].reverse.complement.seq
        ...
        if _down[0].strand == '+':
            ...
            for exon in _down[::-1]:
                downstream_seq += genome_fasta[exon.chrom][exon.start:exon.end].reverse.complement.seq


        seq = upstream_seq + downstream_seq
        title = '>{} {}|{}|{} {}-{} {}|{}'.format(gene_id, '&'.join(upstream_titles), '&'.join(downstream_titles), event_type, bp_up, bp_down, len(upstream_seq), len(seq))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', action='store', dest='input', help="Input VCF file", required=True)
    parser.add_argument('-b', '--bam', action='store', dest='bam', help="SV built BAM file", required=True)
    parser.add_argument('-r', '--ref', action='store', dest='ref', help="reference genome in FASTA format (with fai index)", required=True)
    parser.add_argument('-a', '--alignment_fraction', action='store', dest='alignment_fraction', type=float, help="minimal fraction of aligned part for smith-waterman local alignment (default: %(default)s)", default=0.8)
    parser.add_argument('-n', action='store', dest='mismatch', type=int, help="maximum mismatch bases of pairwise local alignment (default: %(default)s)", default=3)
    ...
    parser.add_argument('-o', '--output', action='store', dest='output', help="output file prefix (default: %(default)s)",default='output')
