...
def extract_splice_sites(file, bin):
    ...
    for feature in gtf_file:
        ...
        if biotype == 'protein_coding':
            ...
            if feature.type == 'gene':
                gene_iv[HTSeq.GenomicInterval(feature.iv.chrom, feature.iv.start-bin, feature.iv.end+bin, '.')] += str(gene_name)
        #if feature.type == 'gene':
    ...
    for trx_id in trx_to_exon:
        ...
        if exon_num == 1:
            cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.start-bin, first_exon.start+bin, '.')] += 'XX'
            cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.end-bin, first_exon.end+bin, '.')] += 'XX'
        elif exon_num == 2:
            if strand == '+':
                cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.start-bin, first_exon.start+bin, '.')] += 'XX'
                cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.end-bin, first_exon.end+bin, '.')] += 'GT'
                cvg[HTSeq.GenomicInterval(last_exon.chrom, last_exon.start-bin, last_exon.start+bin, '.')] += 'AG'
                cvg[HTSeq.GenomicInterval(last_exon.chrom, last_exon.end-bin, last_exon.end+bin, '.')] += 'XX'
            elif strand == '-':
                cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.start-bin, first_exon.start+bin, '.')] += 'XX'
                cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.end-bin, first_exon.end+bin, '.')] += 'CT'
                cvg[HTSeq.GenomicInterval(last_exon.chrom, last_exon.start-bin, last_exon.start+bin, '.')] += 'AC'
                cvg[HTSeq.GenomicInterval(last_exon.chrom, last_exon.end-bin, last_exon.end+bin, '.')] += 'XX'
        else:
            if strand == '+':
                cvg[HTSeq.GenomicInterval(first_exon.chrom, first_exon.start-bin, first_exon.start+bin, '.')] += 'XX'
...
def splicing_confirmation(chrm1, pos1, chrm2, pos2, splice_bin, fastafile, cvg, strand_changed, motif_required=True):
...
def update_breakpoints(bp1_chrm, bp1_pos, bp2_chrm, bp2_pos, bp1_strand, bp2_strand, bp1_mode, bp2_mode, splice_bin, genome_fasta):
    '''
    update the breakpoints of NLS events with canonical splice sites
    keep NLS events with noncanonical splice sites unchanged
    GT-AG;GC-AG;AT-AC
    '''
    tgt_motifs = {1: {'+':{'GT','GC','AT'}, '-':{'AG','AC'}},
        2: {'+':{'AG','AC'}, '-':{'GT','GC','AT'}}}
    ...
    if bp1_mode == 2:
        if bp1_strand == '+':
            boundary_seq1 = genome_fasta[bp1_chrm][bp1_pos-splice_bin:bp1_pos+splice_bin].seq
            #left to right: AG, AC
            ...
            boundary_seq1 = genome_fasta[bp1_chrm][bp1_pos-splice_bin:bp1_pos+splice_bin].reverse.complement.seq
            #right to left: GT, GC, AT
            #print(boundary_seq1)
            bp1_pos_dict['GT'] = splice_site_search(boundary_seq1, 'GT', splice_bin, False)
            #print(bp1_pos_dict)

            bp1_pos_dict['GC'] = splice_site_search(boundary_seq1, 'GC', splice_bin, False)
            bp1_pos_dict['AT'] = splice_site_search(boundary_seq1, 'AT', splice_bin, False)
    elif bp1_mode == 1:
        if bp1_strand == '+':
            boundary_seq1 = genome_fasta[bp1_chrm][bp1_pos-splice_bin:bp1_pos+splice_bin].seq
            #right to left: GT, GC, AT
    ...
    if bp2_mode == 2:
        if bp2_strand == '+':
            ...
            #else:
            boundary_seq2 = genome_fasta[bp2_chrm][bp2_pos-splice_bin:bp2_pos+splice_bin].seq
            #left to right: AG, AC
            ...
            #else:
            boundary_seq2 = genome_fasta[bp2_chrm][bp2_pos-splice_bin:bp2_pos+splice_bin].reverse.complement.seq
            #right to left: GT, GC, AT
            bp2_pos_dict['GT'] = splice_site_search(boundary_seq2, 'GT', splice_bin, False)
            bp2_pos_dict['GC'] = splice_site_search(boundary_seq2, 'GC', splice_bin, False)
            bp2_pos_dict['AT'] = splice_site_search(boundary_seq2, 'AT', splice_bin, False)
    elif bp2_mode == 1:
        if bp2_strand == '+':
            #if TRA:
            #    boundary_seq2 = genome_fasta[bp2_chrm][bp2_pos-1-splice_bin:bp2_pos-1+splice_bin].seq
...
def obtain_bps_shift_len(bp1_dict, bp2_dict, bp1_upstream=True):
    ...
    '''
    if bp1_upstream:
        if 'GT' in bp1_dict and 'AG' in bp2_dict and bp1_dict['GT'] !=-1 and bp2_dict['AG'] !=-1:
            return bp1_dict['GT'], bp2_dict['AG']
        elif 'GC' in bp1_dict and 'AG' in bp2_dict and bp1_dict['GC'] !=-1 and bp2_dict['AG'] !=-1:
            return bp1_dict['GC'], bp2_dict['AG']
        elif 'AT' in bp1_dict and 'AC' in bp2_dict and bp1_dict['AT'] !=-1 and bp2_dict['AC'] !=-1:
            return bp1_dict['AT'], bp2_dict['AC']
        else:
            # No splice site found
            return True, True
    else:
        if 'GT' in bp2_dict and 'AG' in bp1_dict and bp2_dict['GT'] !=-1 and bp1_dict['AG'] !=-1:
            return bp1_dict['AG'], bp2_dict['GT']
        elif 'GC' in bp2_dict and 'AG' in bp1_dict and bp2_dict['GC'] !=-1 and bp1_dict['AG'] !=-1:
            return bp1_dict['AG'], bp2_dict['GC']
        elif 'AT' in bp2_dict and 'AC' in bp1_dict and bp2_dict['AT'] !=-1 and bp1_dict['AC'] !=-1:
...
def detect_fusion_from_cigar(chrm, read, mapq_cutoff, splice_bin, fastafile, cvg, gene_iv, min_dist_between_loci=10000, motif_required=True):
    '''
    ...
    for start,end in intron_positions:
        if end - start >= min_dist_between_loci:
            if strand == '-':
                left_site = fastafile[chrm][end-2-1:end-1].reverse.complement.seq
                right_site = fastafile[chrm][start-1:start+2-1].reverse.complement.seq
            else:
            ...
            if f'{left_site}-{right_site}' in can_sites:
                ...
                _can = 0
            _nls, _anno, __can = splicing_confirmation(chrm, start, chrm, end, splice_bin, fastafile, cvg, False, motif_required)
            _genes = gene_annotation(chrm, start, chrm, end, gene_iv)
            if _nls and strict_inequality(_genes):
                fusion_list.append(('FUS', _anno, __can, [start, end - start, 0, 0], [strand, strand], [*_genes]))
    return fusion_list


def short_TDUP_or_not(chrm, ra_mode, sa_start, sa_end, ins_seq_in_read, fastafile) -> bool:
...
def chimeric_aln_order_finder(aln_list, soft_len_cutoff=30) -> tuple:
    ...
    def select_path_for_one_node(node, paths) -> tuple:
        ...
        for i in paths:
            len_dict[i] = len(i)
        nm_max_len_dict = dict((j, j.nm) for j in len_dict if len_dict[j] == max(len_dict.values()))
    ...
    if len(start_nodes) == 2:
        ...
        if len(candidate_nodes) == 0:
            _is_connected, _, _mode = test_is_connected(start_nodes[0].sms, start_nodes[1].sms)
            if _is_connected:
                reads_pair_mode_dict[(start_nodes[0], start_nodes[1])] = _mode
                end_to_end_chain.append(start_nodes)
        elif len(candidate_nodes) == 1:
            for _node in start_nodes:
                _is_connected, _, _mode = test_is_connected(_node.sms, candidate_nodes[0].sms)
                if _is_connected:
            ...
            while not stop_signal:
                ...
                for _node in candidate_nodes:
                    if count < len(candidate_nodes):
                        _is_connected, _sum_sms, _mode = test_is_connected(tgt_node.sms, _node.sms)
                        if _is_connected:
                            stop_signal = False
                        ...
                        for path in tgt_node.linked_paths:
                            ...
                            _is_connected, _sum_sms, _mode = test_is_connected(path.sms, _node.sms)
                            if _is_connected:
            ...
            end_to_end_chain, reads_pair_mode_dict = optimal_path_selector(lt_node, rt_node, lt_candidate_paths, rt_candidate_paths)
...
def output_bedpe_file(sr_dict, group_dict, prefix, splice_bin):
    ...
    for key in sr_dict:
        ...
        if pos1 - splice_bin > 0 and pos2 - splice_bin > 0:
            output.write(f'{chrm1}\t{pos1-splice_bin}\t{pos1+splice_bin}\t{chrm2}\t{pos2-splice_bin}\t{pos2+splice_bin}\tgroup_{num_of_group}\t{sr}\t{strand1}\t{strand2}\n')
    output.close()
    return output
...
def similar_hit(r1, r2, len_cutoff=10):
    ...
    if r1_type != r2_type:
        ...
        if chrm_a == chrm_A and chrm_b == chrm_B:
            if abs(int(pos_a) - int(pos_A)) <= len_cutoff and abs(int(pos_b) - int(pos_B)) <= len_cutoff:
                return True
            else:
                return False

        elif chrm_a == chrm_B and chrm_b == chrm_A:
            if abs(int(pos_a) - int(pos_B)) <= len_cutoff and abs(int(pos_b) - int(pos_A)) <= len_cutoff:
                return True
            else:
                return False
        else:
...
def vcf_header(output_prefix, bam_header):
    ...
    _aligners = {'CLC','ContextMap2','CRAC','GSNAP','HISAT','HISAT2',
            'MapSplice2', 'Novoalign', 'OLego', 'RUM', 'SOAPsplice',
            'STAR', 'Subread','TopHat', 'TopHap2',
            'bwa', 'bowtie', 'bowtie2', 'minimap2'}
    ...
    header.append('##INFO=<ID=PSO,Number=1,Type=Float,Description="Estimated allele frequency in the range (0,1], representing the ratio of reads showing the alternative allele to all reads">')
    header.append(
...
def sv_checker(read, ref_site, mapq_cutoff, sv_len_cutoff):
    ...
    if len(chimeric_aln) == 1:
        ...
        chr_sa, pos_sa, strand_sa, cigar_sa, mapq_sa, nm_sa = multiple_sa_tag_selector(chimeric_aln).split(',')

    if int(mapq_sa) < mapq_cutoff:
        return False
    sv_type, sv_anno_and_can, sv_pos, sv_length, read_mode, sa_mode, strands, genes = read.alignment.get_tag(
        'SV').split(',')
